...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		// 1、先取出 bean标签的 id 和 name
		String id = ele.getAttribute(ID_ATTRIBUTE);
		String nameAttr = ele.getAttribute(NAME_ATTRIBUTE);

		// 2. bean 的name属性，可以写多个值，使用,或者；分隔
		// 这个使用，或；将 bean的 name属性值进行拆分，转换成一个集合，作为 bean的别名alias
		List<String> aliases = new ArrayList<>();
		if (StringUtils.hasLength(nameAttr)) {
			String[] nameArr = StringUtils.tokenizeToStringArray(nameAttr, MULTI_VALUE_ATTRIBUTE_DELIMITERS);
			aliases.addAll(Arrays.asList(nameArr));
		}
		// 3 如果bean的 id 为空，就从 bean 的别名alias集合中取出第一个值，作为 bean的name
		String beanName = id;
		...
		// 4 开始解析bean标签，并将结果封装成 AbstractBeanDefinition对象
		...
		if (beanDefinition != null) {
			...
			// 根据解析的 AbstractBeanDefinition、beanName 和 alias，创建一个 BeanDefinitionHolder
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		// bean中包含 class属性，就获取 bean 的 class属性的值
		...
		try {
			// 首先，根据 class属性和 parent属性，创建 AbstractBeanDefinition 对象
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);

			// 解析 bean标签的其他属性（autowire、scope等等），然后设置到 AbstractBeanDefinition 中
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));

			// 解析 bean标签的 子标签元素，然后将结果设置到 AbstractBeanDefinition
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) {
				...
				// 获取到 meta的 key 和 value属性
				// 然后构造 BeanMetadataAttribute 对象
				BeanMetadataAttribute attribute = new BeanMetadataAttribute(key, value);

				attribute.setSource(extractSource(metaElement));
				// 将 BeanMetadataAttribute 添加到 BeanMetadataAttributeAccessor 中
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, LOOKUP_METHOD_ELEMENT)) {
				...
				// 将 name 和 bean 封装到 LookupOverride 对象中
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						// 解析constructor-arg标签的所有属性
						Object value = parsePropertyValue(ele, bd, null);
						// 获取到值后，将值封装到 ConstructorArgumentValues.ValueHolder
	...
	public void parsePropertyElement(Element ele, BeanDefinition bd) {
		...
		try {
			// 如果已经解析了一个相同名称的 property值，再次解析就会报错
			...
			// 将值封装到 PropertyValue中
			PropertyValue pv = new PropertyValue(propertyName, val);

			parseMetaElements(ele, pv);
			pv.setSource(extractSource(ele));
			// 将值添加到 BeanDefinition中
