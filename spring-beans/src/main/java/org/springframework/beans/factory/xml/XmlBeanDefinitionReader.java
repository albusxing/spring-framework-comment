...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) {
			...
			// 资源加载器为空，就是 DelegatingEntityResolver
			// 不为空，就是 ResourceEntityResolver
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// 先将 资源 包装成 带字符集和编码类型的资源，然后加载 BeanDefinition
		// Spring容器中 bean会被封装成 BeanDefinition对象
		//
	...
	/**
	 * 实际的加载 BeanDefinition方法
	 * 1、将资源对象Resource，包装成 Document对象
	 * 2、解析Document对象 成为 BeanDefinition，然后注入到 Spring 容器中
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			// 根据 inputSource 和 resource 创建 xml文件对应的 Document
			Document doc = doLoadDocument(inputSource, resource);
			// 解析 Document对象，并注册 Bean到 Spring 容器中
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// 又是使用 documentLoader 对象来加载 XML文件
		// 传入 EntityResolver，用来解析XML文件
		//
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 创建documentReader，使用的反射机制
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 获取spring容器中已经注册的bean的数目
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 使用 documentReader 解析 document对象，然后将解析出来的 bean注册到spring容器中
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		// 计算出本次注册到 spring容器中的bean的数目
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		// 内部使用的的反射机制
		// 创建一个 BeanDefinitionDocumentReader对象，它的类型为 DefaultBeanDefinitionDocumentReader
